...
def main(args):
    ...
    for name,p in model.named_parameters():
        #print("name",name,p.shape)
        states[name]=p
    if args.freeze:
        for name,p in model_convnext.named_parameters():
            if name in states and p.shape==states[name].shape:
    ...
    for name,p in model_convnext.named_parameters():
        if name in states and p.shape==states[name].shape:
            p.data=states[name].data.clone().detach()
            print("name loaded in",name,p.shape)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            use_amp=args.use_amp, mask=mask,T=args.T,hard=args.hard
        )
