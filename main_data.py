...
def get_args_parser():
    ...
    parser.add_argument('--remove_n', default=0.5, type=float)
...
def main(args):
    ...
    if args.model=='convnext':
        ...
        # a=model(x)
        # print(model.stage_features[0].shape)
        # print(model.stage_features[1].shape)
        # print(model.stage_features[2].shape)
        # print(model.stage_features[3].shape)
        # return 0
    ...
    if args.model_s=='resnet50':
        ...
        #pass
        #model_convnext=convnextv2_tiny()\
    ...
    if args.model!='vit' and args.model!='swin' and args.model!='vitbase' and args.model!='vitdeit' and args.model!='cswin': 
        print("model",args.model,"Decom",args.Decom)
    ...
    # dataset_train = data_utils.Subset(dataset_train, train_indx)
    # print ("dataset_train.data ",dataset_train.data )
    def evaluate_for_prune(model,dataset_train, device,example_stats_train):
        ...
        train_indx = np.array(range(len(dataset_train.samples)))
        ...
        trainset_permutation_inds = npr.permutation(
            np.arange(len(dataset_train)))   #numpy random permutation
        batch_size = 100


        for batch_idx, batch_start_ind in enumerate(
                range(0, len(dataset_train), batch_size)):
                print ("batch_idx",batch_idx,"batch_start_ind",batch_start_ind)
            # Get trainset indices for batch
                ...
                for j, index in enumerate(batch_inds):
                    ...
                    # print ("output_correct_class",output_correct_class)
    ...
    evaluate_for_prune( model,dataset_train, device,example_stats_train)

    # print ("example_stats_train",example_stats_train)


    def compute_forgetting_statistics(diag_stats):
        ...
        for example_id, example_stats in diag_stats.items():
            ...
            if not isinstance(example_id, str):
                ...
                margins = np.array(
                    example_stats[2][-1])
                margins_per_presentation[example_id] = margins


        return margins_per_presentation


    def sort_examples_by_margin(margins_per_presentation):
        
        # Initialize lists
        example_original_order = []
        example_stats = []

        for example_id in margins_per_presentation.keys():

            # Add current example to lists
            example_original_order.append(example_id)
            example_stats.append(margins_per_presentation[example_id])


  
        return np.array(example_original_order)[np.argsort(
            example_stats)[::-1]], np.sort(example_stats)[::-1], 0


    margins_per_presentation = compute_forgetting_statistics(example_stats_train)


    ordered_examples, ordered_values, num_unforget = sort_examples_by_margin(margins_per_presentation)
    ...
    print('epoch before len(train_dataset.samples)', len(dataset_train.samples))
    elements_to_remove = np.array(
        ordered_examples)[:int(args.remove_n*len(train_indx))]
    ...
    dataset_train.sample=[ dataset_train.samples[i] for i in train_indx  ]
