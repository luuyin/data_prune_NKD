...
def auto_load_model1(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.auto_resume:
        import glob
        all_checkpoints = glob.glob(os.path.join(output_dir, 'checkpoint-*_T.pth'))
        latest_ckpt = -1
        for ckpt in all_checkpoints:
            t = ckpt.split('-')[-1].split('_')[0]
            if t.isdigit():
                latest_ckpt = max(int(t), latest_ckpt)
        if latest_ckpt >= 0:
            args.resume = os.path.join(output_dir, 'checkpoint-%d_T.pth' % latest_ckpt)
...
def auto_load_model(args, model, model_without_ddp, optimizer, loss_scaler, model_ema=None):
    ...
    if args.auto_resume:
        import glob
        all_checkpoints = glob.glob(os.path.join(output_dir, 'checkpoint-*_S.pth'))
        latest_ckpt = -1
        for ckpt in all_checkpoints:
            t = ckpt.split('-')[-1].split('_')[0]
            if t.isdigit():
                latest_ckpt = max(int(t), latest_ckpt)
        if latest_ckpt >= 0:
            args.resume = os.path.join(output_dir, 'checkpoint-%d_S.pth' % latest_ckpt)
        print("Auto resume checkpoint: %s" % args.resume)
