    ...
    for data_iter_step, (samples, targets) in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        ...
        if use_amp:
            with torch.cuda.amp.autocast():
                ...
                if 'FD' in args.distill_type :
                ...
                elif args.distill_type=='LayerFeature':
                    loss=criterion(output, targets)
                    assert args.model_s=='resnet50'
                    out_s=model_convxt.module.project()
                    out_t=model.module.stage_features
                    losses_ft=0
                    for i in range(args.feature_n):
                        if args.model=='swin':
                            temp=out_t[i]
                            temp=temp.reshape(temp.size(0),swin_kernel_dict[i],swin_kernel_dict[i], temp.size(2))
                            temp=temp.transpose(2, 3).transpose(1, 2)
                            out_t[i]=temp
                        elif args.model=='vit':
                            temp=out_t[j]
                            temp=temp.reshape(temp.size(0),vit_kernel_dict[i],vit_kernel_dict[i], temp.size(2))
                            temp=temp.transpose(2, 3).transpose(1, 2)
                            out_t[j]=temp
                        if args.target_Lnorm:
                            out_t[i]=Lnorms[i](out_t[i])     ##normalize
                        losses_ft=losses_ft+loss_feat(out_s[i],out_t[i].detach())
                    losses_ft=losses_ft/args.feature_n
                    loss=loss+losses_ft*args.lr_fd
                elif args.distill_type=='LayerFeatureKD':
                    loss=loss_kd(output,targets,output_t,T,hard=hard,alpha=args.alpha)
                    assert args.model_s=='resnet50'
                    out_s=model_convxt.module.project()
                    out_t=model.module.stage_features
                    ...
                    for i in range(args.feature_n):
                        ...
                        if args.model=='swin':
                            ...
                            out_t[j]=temp
                        if args.target_Lnorm:
                                out_t[j]=Lnorms[i](out_t[j])     ##normalize
                        losses_ft=losses_ft+loss_feat(out_s[i],out_t[j].detach())
                    losses_ft=losses_ft/args.feature_n
                    loss=loss+losses_ft*args.lr_fd
                elif args.distill_type=='LayerFeatureNKD':
                    loss=nkdloss(output,output_t,targets)
                    assert args.model_s=='resnet50'
                    out_s=model_convxt.module.project()
                    out_t=model.module.stage_features
                    ...
                    losses_ft=losses_ft/args.feature_n
                    loss=loss+losses_ft*args.lr_fd
                elif 'MGD' in args.distill_type:
                    loss=criterion(output, targets)
                    if args.model=='swin':
                        target=model.module.feature.detach()
                        target=target.reshape(target.size(0),7,7, target.size(2))
                        target=target.transpose(2, 3).transpose(1, 2)
                        loss_dis=MGDloss(model_convxt.module.feature,target)
                    elif args.model=='vit' or args.model=='vitdeit':
                        target=model.module.feature.detach()
                        target=target.reshape(target.size(0),14,14, target.size(2))
                        target=target.transpose(2, 3).transpose(1, 2)
                        ...
                        loss_dis=MGDloss(model_convxt.module.feature,target)
                            ...
                            #print("student shape",model_convxt.module.feature.shape)
                            #loss_dis=loss_feat(model_convxt.module.project_slak(),target).mean()
                            loss_dis=MGDloss(model_convxt.module.feature,target)
                    else:
                        loss_dis=MGDloss(model_convxt.module.feature,model.module.feature.detach())
                    if 'NKD' in args.distill_type:
                        loss=nkdloss(output,output_t,targets)+loss_dis
                    else:    
                        if 'KD' in args.distill_type:
                            loss=loss_kd(output,targets,output_t,T,hard=hard,alpha=args.alpha)+loss_dis
                        else:   
                            loss=loss+loss_dis                
                else:
                    assert False
