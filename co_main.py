...
from co_engine import train_one_epoch, evaluate
from utils import NativeScalerWithGradNormCount as NativeScaler
from sparse_core import Masking, CosineDecay
import warnings

warnings.filterwarnings("ignore")
import co_utils as utils
import models.SLaK
...
def main(args):
    ...
    #     optimizer=optimizer_t, loss_scaler=loss_scaler, model_ema=model_ema)

    # utils.auto_load_model(
    #     args=args, model=model_convnext, model_without_ddp=model_convnext_without_ddp,
    #     optimizer=optimizer, loss_scaler=loss_scaler, model_ema=model_ema)
    # print("loaded at:",args.start_epoch)
    ...
    if args.sparse:
        ...
        mask = Masking(optimizer_t, train_loader=data_loader_train, prune_mode=args.prune, prune_rate_decay=decay, growth_mode=args.growth, redistribution_mode=args.redistribution, args=args)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model,model_convnext, criterion, data_loader_train, optimizer,optimizer_t,
            device, epoch, loss_scaler, args.clip_grad, model_ema, mixup_fn,
            log_writer=log_writer, wandb_logger=wandb_logger, start_steps=epoch * num_training_steps_per_epoch,
            lr_schedule_values=lr_schedule_values, wd_schedule_values=wd_schedule_values,
            num_training_steps_per_epoch=num_training_steps_per_epoch, update_freq=args.update_freq,
            use_amp=args.use_amp, mask=mask,T=args.T
        )
        if args.output_dir and args.save_ckpt:
            if (epoch + 1) % args.save_ckpt_freq == 0 or epoch + 1 == args.epochs:
                utils.save_model(
                    args=args, model=model_convnext, model_without_ddp=model_convnext_without_ddp, optimizer=optimizer,
                    loss_scaler=loss_scaler, epoch=str(epoch)+"_S", model_ema=model_ema)
                utils.save_model(
                    args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer_t,
                    loss_scaler=loss_scaler, epoch=str(epoch)+"_T", model_ema=model_ema)


        if data_loader_val is not None:
            test_stats_s = evaluate(data_loader_val, model_convnext, device, use_amp=args.use_amp)
            test_stats_t = evaluate(data_loader_val, model, device, use_amp=args.use_amp)
            print(f"Accuracy of the model on the {len(dataset_val)} test images: {test_stats_s['acc1']:.1f}%")
            print(f"Accuracy of the model on the {len(dataset_val)} test images: {test_stats_t['acc1']:.1f}%")
            if max_accuracy_s < test_stats_s["acc1"]:
                max_accuracy_s = test_stats_s["acc1"]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model_convnext, model_without_ddp=model_convnext_without_ddp, optimizer=optimizer,
                        loss_scaler=loss_scaler, epoch="best_s", model_ema=model_ema)
            if max_accuracy_t < test_stats_t["acc1"]:
                max_accuracy_t = test_stats_t["acc1"]
                if args.output_dir and args.save_ckpt:
                    utils.save_model(
                        args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer_t,
                        loss_scaler=loss_scaler, epoch="best_t", model_ema=model_ema)
            print(f'Max accuracy_t: {max_accuracy_s:.2f}%')

            if log_writer is not None:
                log_writer.update(test_acc1_s=test_stats_s['acc1'], head="perf", step=epoch)
                log_writer.update(test_acc5_s=test_stats_s['acc5'], head="perf", step=epoch)
                log_writer.update(test_loss_s=test_stats_s['loss'], head="perf", step=epoch)
                log_writer.update(test_acc1_t=test_stats_t['acc1'], head="perf", step=epoch)
                log_writer.update(test_acc5_t=test_stats_t['acc5'], head="perf", step=epoch)
                log_writer.update(test_loss_t=test_stats_t['loss'], head="perf", step=epoch)
